...
function expandtok ($token,$stream)
{
  global $commands;
  ...
  if ($firstchar == "\\")
      ...
      if (array_key_exists($command,$commands))
	{
	  ...
	  for ($i = 0;$i < $args;$i++)
	    {
	      list($arg,$stream) = nexttok($stream);
	      $defn=str_replace("#" . ($i+1), $arg, $defn);
	    }
	  // return expanded command
	  return array(1,$defn,$stream);
	}
      else
	{
	  // command is not known, just return it unexpanded
	  return array(0,$token,$stream);
	}
    }
  elseif ($firstchar == "{")
    {
  ...
  while ($latex)
    {
      ...
      list($token,$rest) = nexttok($latex);

      // expand token
      list($mod,$extoken,$rest) = expandtok($token,$rest);
